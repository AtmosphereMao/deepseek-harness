# Agent Note：整页图片拖放、文件选择上传、上限投影预检与缩略图平铺
...
issue #2248 的第二步对齐，接在[附件展示 note](2026-08-11-web-attachment-display-alignment.md) 之后（其附件栏、toast 与原子组件包的决策继续有效；本 note 取代其中历史画廊几何与灯箱 backdrop 的具体规格）。与 DeepSeek Chat 相比剩下的差距：图片只能拖到 composer 卡片上——拖到聊天记录区会让浏览器直接导航到文件；灯箱关闭钮是裸 `×` 文本字符（button 不继承字体，且该字形的墨迹在行框中心之上，因此明显偏斜），backdrop 用 `color-mix(label-primary 74%)`，dark 下反转成刺眼的白色蒙层；一条消息的多张图各自以最大 240px 的块竖着堆叠，因为画廊容器本身被钉在 240px；客户端完全不执行也不展示图片限额——用户可以攒 50 张图，直到提交后收到原始的 `attachment-error (TOO_MANY_IMAGES)` toast，眼看附件栏清空又回滚。加入也缺少触屏路径：手机浏览器既没有剪贴板文件粘贴，也没有拖拽手势，移动端完全无法加图，而桌面端可以粘贴。
...
**文件选择上传（移动端加入）。** 一个专用上传按钮放在 composer 工具栏最前（命令菜单 `+` 之前），打开一个隐藏的 `<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" multiple>`；选中项走同一个 `intakeImages` 包装，输入在选中后清空 value，这样再次选中同一文件仍会触发 `change`。按钮沿用拖放遮罩的同一 `canAcceptDrop` 门控（锁定、忙碌或没有 attachment 服务）来禁用，于是粘贴、拖放、文件选择三条通道共用一份加入与一份门控规则。按钮是通往隐藏输入的唯一指针；`display: none` 让选择器不出现在无障碍树里，而 `accept` 把系统选择器预过滤为四种支持的格式。
