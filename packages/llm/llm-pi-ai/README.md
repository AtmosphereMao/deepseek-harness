...
`[text]` is the absence of a declaration rather than a guess at the endpoint, which is why the fallback here is conservative where the capacity fallbacks are merely plausible. Nothing interrogates a gateway for what it accepts, and the two wrong answers do not cost the same: a text-only route replaces each image block with a placeholder naming the attachment — mirroring the DeepSeek serializer — so the model still learns an image exists and can hand describing it to a vision-capable subagent, while over-claiming admits an image the provider then rejects mid-turn — after the message is durable, which leaves the session repeating a request that cannot succeed.
