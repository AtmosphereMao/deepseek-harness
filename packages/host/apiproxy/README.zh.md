...
子代理路由是与之并行的按会话选择。`session.models` 携带 `subagent`（进程内的子代理模型选择，未设置时为 `null`），`session.selectSubagentModel` 校验路由并记录该选择，供该会话 agent 派生的子代理使用。与 `session.selectModel` 不同，它不保存部署默认值，也不改动父请求：该选择是进程本地的，重启后恢复为工具的组合 `agentOptions`。
