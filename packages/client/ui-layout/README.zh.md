...
在 640px 以下的两种折叠状态下，侧栏都不占用网格轨道，因此中间栏始终横跨视口：关闭时，它以单个切换按钮浮动在中间栏前角（框架把该按钮宽度发布为 `--dsh-frame-leading-inset`，从同一边缘开始的各排 chrome 据此留白）；重新展开时，它作为抽屉浮动并带点击关闭的遮罩，而不是把中间栏挤到行内。
...
- **手机抽屉在导航后保持打开**——从抽屉选择会话不会关闭它；遮罩或折叠切换按钮负责关闭。
