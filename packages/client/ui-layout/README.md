...
Below 640px the sidebar takes no grid track in either fold state, so the center always spans the viewport: closed, it floats as a single toggle button over the center's leading corner (the frame publishes that button's width as `--dsh-frame-leading-inset`, which row chrome starting at the same edge pads by); re-expanding floats it as a drawer with a tap-to-close scrim instead of squeezing the center inline.
