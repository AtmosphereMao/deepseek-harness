...
On phone viewports the layout owner share reports `floating`: the collapsed state renders as a single open-the-drawer button rather than the rail, since painting the rail's other controls over the conversation would obscure the chat, and each control stays one tap away inside the drawer. Expanded-while-floating is the ordinary wide shell.
