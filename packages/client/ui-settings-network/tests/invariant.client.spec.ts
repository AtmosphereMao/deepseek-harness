...
import * as NetworkInvariant from '@deepseek-ai/dsh-client-ui-settings-network/invariant'
...
describe('invariant companion', () => {
  it('reserves package ownership with an empty installer', async () => {
    ...
    await expect(ctx.plugin(NetworkInvariant).await()).resolves.toBeDefined()
  })

  it('has an empty node half', async () => {
    const { apply } = await import('@deepseek-ai/dsh-client-ui-settings-network')
