...
| `agentOptions` | 传给具体提供方的子 agent `provider`、`model` 和正整数 `maxTokens`；进程内提供方会用显式值覆盖继承的父级选项。按会话的 Web 子代理模型选择（composer 的「子代理模型」面板）会在派生时覆盖 `provider`/`model`，同时仍继承 `maxTokens`；没有选择时该值保持不变。 |
...
当提供方存在时，以当前实例配置的名称公开已生成的默认 [`subagent` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-subagent)。提供方是否继承上下文会改变工具描述和提示词描述。声明 `imagePrompt` 能力的提供方会添加 `image_attachment_ids`，它是父对话中已存在图片的 attachment id 可选列表；每个 id 会被解析为对应的图片块并注入子 agent 的提示词。纯文本提供方会省略该参数，并拒绝强行传入的值，因为服务不会接受自己无法送达的带图片提示词。启用后台模式会添加 `run_in_background`：可继续模式会记录其默认值为 `true`、运行时结算通知与显式前台覆盖；一次性模式会记录其默认值为 `false`，以及用 `job_output` 收集或用 `job_kill` 停止的 job id。当工具在本次组装的作用域中可见时，一个 `tool:<toolName>` 系统提示词 section 会承载该实例应得的指引：可继续模式指示模型同时启动相互独立的委派、在它们运行时继续工作，并且仅当下一步动作依赖结果时选择前台；`imagePrompt` 提供方还会补充——`[image attached: …]` 占位符意味着模型自己看不到这张图，因此应当带上 `image_attachment_ids` 委派并转述子 agent 的答复，而不是要求用户切换模型。工具限制会同时移除其 schema 和这段指引。
...
调用会保留描述和提示词；当设置 `image_attachment_ids` 时，其解析出的图片块会排在提示词之前成为子 agent 的内容，而对话中找不到对应图片的 id 会使调用失败。成功时只包含子 agent 的最终文本；其他结果变为 `Error: <message>`。子 agent 中间步骤不会进入父级。
