...
| `agentOptions` | Provider-specific child `provider`, `model`, and positive `maxTokens`; the in-process provider treats explicit values as overrides of inherited parent options. A per-session Web subagent-model selection (the composer's `子代理模型` pane) overrides `provider`/`model` at spawn while still inheriting `maxTokens`; no selection leaves this value unchanged. |
...
The generated default [`subagent` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-subagent) under this instance's configured name while its provider exists. Provider context inheritance changes the tool and prompt descriptions. A provider declaring the `imagePrompt` capability adds `image_attachment_ids`, an optional list of image attachment ids already in the parent conversation; each id is resolved to its image block and injected into the child prompt. A text-only provider omits the parameter and rejects a forced one, because the service refuses an image-bearing prompt it could not deliver. Enabled background mode adds `run_in_background`: continuable mode documents its `true` default, runtime settlement notice, and explicit foreground override, while one-shot mode documents its `false` default and the job id collected with `job_output` or stopped with `job_kill`. While the tool is visible in an assembly's scope, a `tool:<toolName>` system-prompt section carries the guidance its instance earns: continuable mode tells the model to start independent delegations together, keep working while they run, and choose foreground only when its next action depends on the result; an `imagePrompt` provider adds that an `[image attached: …]` placeholder means its own model cannot see the picture, so it should delegate with `image_attachment_ids` and report the child's answer instead of asking the user to switch models. A tool restriction removes both its schema and this guidance.
