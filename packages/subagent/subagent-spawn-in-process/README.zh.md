...
spawn 声明 `{ outputSchema: true, depthLimit: true, toolFilter: true, persona: true, imagePrompt: true }`，因为它控制子 agent 的创建窗口，能够强制执行全部四项启动期功能。`imagePrompt` 成立是因为子 agent 运行在本进程内、原样收到 `request.prompt`，图片块能不经改动到达其模型。
