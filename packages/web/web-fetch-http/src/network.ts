 ...
 * @param transport - optional shared transport; a custom one owns the connection
 *   (and, for a configured proxy, the DNS resolution), so the pinned lookup only
...
export async function requestPinned(
  ...
  signal: AbortSignal,
  transport: (input: string | URL, init?: RequestInit) => Promise<Response> = fetch,
): Promise<PinnedResponse> {
  // A custom transport (the shared proxy transport) owns the connection and its
  // DNS resolution, so the pinned lookup cannot apply; the validated addresses
  // only guard the direct path below.
  if (transport !== fetch) {
    const response = await transport(url, { method: 'GET', redirect: 'manual', headers, signal })
  ...
  try {
    // Undici's fetch Response carries a narrower body stream type than the
    // DOM `Response` this module advertises; the value is the same object at
    // runtime, so the cast keeps one response type across both transport paths.
    const response = await undiciFetch(url, { method: 'GET', redirect: 'manual', headers, signal, dispatcher }) as unknown as Response
